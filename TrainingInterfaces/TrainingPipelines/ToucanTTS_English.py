...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume, use_wandb, wandb_resume_id):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join(MODELS_DIR, "ToucanTTS_English")  # RENAME TO SOMETHING MEANINGFUL FOR YOUR DATA
    ...
    LJ10_datasets = list()
    LJ10_datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_lj10(),
                                                    corpus_dir=os.path.join(PREPROCESSING_DIR, "LJ10"),
                                                    lang="en",fine_tune_aligner=False,ctc_selection=False))

    all_train_sets.append(ConcatDataset(LJ10_datasets))
