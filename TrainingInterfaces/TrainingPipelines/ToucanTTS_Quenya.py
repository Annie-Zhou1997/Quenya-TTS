...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume, use_wandb, wandb_resume_id):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join(MODELS_DIR, "ToucanTTS_Quenya")  # RENAME TO SOMETHING MEANINGFUL FOR YOUR DATA
    ...
    quenya_datasets = list()
    quenya_datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_Atanquesta(),
                                                    corpus_dir=os.path.join(PREPROCESSING_DIR, "Atanquesta"),
                                                    lang="qya",ctc_selection=False)) #fine_tune_aligner=False,
                                                    
    quenya_datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_Glaemscrafu(),
                                                    corpus_dir=os.path.join(PREPROCESSING_DIR, "Glaemscrafu"),
                                                    lang="qya",ctc_selection=False)) #fine_tune_aligner=False,
    all_train_sets.append(ConcatDataset(quenya_datasets))
    ...
    train_loop(net=model,
               ...
               eval_lang="qya",  # THE LANGUAGE YOUR PROGRESS PLOTS WILL BE MADE IN
               warmup_steps=500,
